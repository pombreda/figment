...
class DatabaseUpdater:
    ...
    def finalize_components(self, distro):
        distro_name = distro.get_name()
        print("Adding data for: %s" % (distro_name))

        for cdata in self.components.values():
            cpt = cdata['cpt']
            pkgname = cdata.get(distro_name, None)

            # is the component present in our distro?
            if not pkgname:
                continue
            ...
            if (not dbcpt.highest_version) or (cdata['version'] > LooseVersion(dbcpt.highest_version)):
            ...
            pkgs = distro.get_packages_info(pkgname)
            for pkg in pkgs:
                dbdistro = db.session.query(Distribution).filter_by(codename=pkg.codename, name=distro_name).first()
                if not dbdistro:
                   continue
    ...
    def import_data(self):
        ...
        for distro in distros:
            ...
            for release in distro.get_releases():
                ...
                db.session.add(d)
                db.session.flush()
            # create a cache of component data
            self.build_component_cache(distro)
        # finalize and commit data
        for distro in distros:
            self.finalize_components(distro)
        db.session.commit()
